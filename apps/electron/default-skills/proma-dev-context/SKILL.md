...
`PROJECT.md` 是常驻入口（精简、导航层），`dev.md` 是会变的实时工作日志（跨会话）。分工：长期**护栏/方向**进 `PROJECT.md`（精简成规则 + 指针），决策的**过程/机制**进 `devlogs/`，在途任务状态进 `dev.md`。
...
- **路由**（产出归位；关键是区分**规则 vs 记录**）：
  - 长期**护栏/规则** + 持久事实（未来每次都要遵守 / 必须知道）→ `PROJECT.md`，**精简成一句 + 指针**
  - 某次决策/改动的**过程、机制、理由、commit hash、"已做 X" 叙述** → `devlogs/`（PROJECT.md 只留一句指针，不复述）
  - 在途任务状态 / 下一步 → `dev.md`
  - 命令 / 端口 / 路径 / 版本 / 环境坑 → `env.md`
  - 短踩坑 → `PITFALLS.md`
  - 调研 / 分析 / 方案 → `notes/YYYY-MM-DD-主题.md`，并在 `PROJECT.md` 知识索引登记链接
- **自检（防 PROJECT.md 变成变更日志）**：写进「关键约定与决策」前问——这是"未来必须遵守的规则 / 必须知道的事实"，还是"记录某事怎么发生的"？前者留（精简 + 指针），后者进 devlog。条目里出现 commit hash / 具体日期 / "已加入/已 push" / 机制步骤链，基本就是 devlog 内容混进来了。
- **收尾 / 交接**：更新 `dev.md`（`Doing` → `Done` 或 `Blocked`，写清下一步）；给活跃 devlog 收尾（状态、产出、验证、遗留）；**只有长期护栏/方向变了**才动 `PROJECT.md`。
